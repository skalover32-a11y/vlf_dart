...
void _validateRules(String config, {required bool expectRF, required bool expectExcl}) {
  final lines = config.split('\n');
  final hasRFRules = lines.any((l) => l.contains('DOMAIN-SUFFIX,ru,DIRECT'));
  final hasExcl = lines.any((l) => l.contains('kinopoisk.ru') || l.contains('chrome.exe'));
  ...
  print('✅ Валидация пройдена: РФ=$expectRF, Исключения=$expectExcl, MATCH=true');
}

void _validateNoDuplicates(String config, String domain) {
  final lines = config.split('\n');
  final domainRules = lines.where((l) => l.contains('DOMAIN-SUFFIX,$domain,DIRECT')).toList();
  
  if (domainRules.length > 1) {
    throw Exception('❌ Найдено ${domainRules.length} дубликатов для $domain:\n${domainRules.join("\n")}');
  }
