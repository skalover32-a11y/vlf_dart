...
void main() async {
  ...
  final firstRule1 = lines1
      .skip(rulesStart1 + 1)
      .firstWhere((l) => l.trim().startsWith('- '), orElse: () => '');
  assert(
    firstRule1.contains('GEOIP,private'),
  ...
  final firstRule2 = lines2
      .skip(rulesStart2 + 1)
      .firstWhere((l) => l.trim().startsWith('- '), orElse: () => '');
  assert(
    firstRule2.contains('PROCESS-NAME'),
  ...
  final domainRule = lines2
      .skip(rulesStart2 + 1)
      .firstWhere(
        (l) =>
            l.contains('DOMAIN-SUFFIX') &&
            (l.contains('google.com') || l.contains('youtube.com')),
        orElse: () => '',
      );
  assert(
    domainRule.isNotEmpty && domainRule.contains('google.com'),
  ...
  final privateRule = lines2
      .skip(rulesStart2 + 1)
      .firstWhere((l) => l.contains('GEOIP,private'), orElse: () => '');
  final privateIndex = lines2.indexOf(privateRule);
  final domainIndex = lines2.indexOf(domainRule);
  assert(
    privateIndex > domainIndex,
  ...
  final hasRuRules = lines3.any(
    (l) => l.contains('DOMAIN-SUFFIX,ru,DIRECT-GROUP'),
  );
  assert(!hasRuRules, '❌ ОШИБКА: В глобальном режиме НЕ должно быть РФ-правил');

  // Проверка: исключения должны быть ПЕРВЫМИ
  final firstRule3 = lines3
      .skip(rulesStart3 + 1)
      .firstWhere((l) => l.trim().startsWith('- '), orElse: () => '');
  assert(
    firstRule3.contains('PROCESS-NAME,telegram.exe'),
  ...
  assert(
    hasFakeIpRu && hasFakeIpSu && hasFakeIpRf,
  ...
  assert(hasStrictRoute, '❌ ОШИБКА: TUN должен содержать strict-route: true');
  ...
  print(
    '  4. РФ-блок (если включен): .ru/.su/.рф + GEOIP,RU + 2ip.ru → DIRECT-GROUP',
  );
